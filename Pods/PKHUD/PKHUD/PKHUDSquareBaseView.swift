...
//        addSubview(subtitleLabel)
    ...
    open override func layoutSubviews() {
        ...
        if (titleLabel.text != nil){
            titleLabel.frame = CGRect(origin: CGPoint(x: originX, y: 0.0), size: CGSize(width: viewWidth, height: quarterHeight + 15))
            imageView.frame = CGRect(origin: CGPoint(x: originX, y: quarterHeight), size: CGSize(width: viewWidth, height: halfHeight + 20))
        }else {
