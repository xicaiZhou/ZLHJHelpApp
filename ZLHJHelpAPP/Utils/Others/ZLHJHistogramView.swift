...
class ZLHJHistogramView: UIView {
...
extension ZLHJHistogramView {
    ...
    fileprivate func drawAxis() {
        ...
        path.addLine(to: CGPoint(x: 0, y: _totalHeight!+20))
        path.addLine(to: CGPoint(x: _totalWidth!, y: _totalHeight!+20))
        // 画左上角的箭头
        path.move(to: CGPoint(x: -5, y: -15))
        path.addLine(to: CGPoint(x: 0, y: -20))
        path.addLine(to: CGPoint(x: 5, y: -15))
        // 画右上角的箭头
        path.move(to: CGPoint(x: _totalWidth! - 5, y: (_totalHeight!+20) - 5))
        path.addLine(to: CGPoint(x: _totalWidth!, y: (_totalHeight!+20)))
        path.addLine(to: CGPoint(x: _totalWidth! - 5, y: (_totalHeight!+20) + 5))
        ...
        for i in 0..<yAxisCount {
            ...
            // 绘制y轴的刻度
            path.move(to: CGPoint(x: 0, y: CGFloat(i)*((_totalHeight!+20)/CGFloat(yAxisCount))))
            path.addLine(to: CGPoint(x: 5, y: CGFloat(i)*((_totalHeight!+20)/CGFloat(yAxisCount))))
        }
        
        // 画x轴的刻度
        for i in 0..<xValueArr!.count {
            let startPoint = CGPoint(x: CGFloat(i) * (barWidth + gapWidth) + gapWidth + barWidth/2, y: (_totalHeight!+20))
            let value = NSValue.init(cgPoint: startPoint)
            barsStartPointsArr.add(value)
            path.move(to: startPoint)
            path.addLine(to: CGPoint(x: CGFloat(i) * (barWidth + gapWidth) + gapWidth + barWidth/2, y: (_totalHeight!+20)-5))
        }
        
        let lineLayer = CAShapeLayer()
        lineLayer.strokeColor = systemColor.cgColor
...
//            let yAxis = yScaleValue * CGFloat(i)
            let label = UILabel(frame: CGRect(x: -5, y: CGFloat((yAxisCount - i)) * ((_totalHeight!+20)/CGFloat(yAxisCount)) - 10, width: -40, height: 20))
//            let str = ("\(yAxis)")
...
//            label.text = ("团队")
    ...
    fileprivate func addHistograms() {
        for i in 0..<yValueArr!.count {
            ...
            // 算出每个矩形的高度
            let barHeight:CGFloat = y / (CGFloat(yAxisCount) * yScaleValue) * (_totalHeight!+20)
            // 计算起始点
...
//            }
            barLayer.strokeColor = UIColor.randomColor.cgColor
...
//        addYAxisLabs()
    ...
    fileprivate func animationWithDuration(duration: CFTimeInterval) -> (CABasicAnimation) {
        ...
        anmiation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: "easeOut"))
