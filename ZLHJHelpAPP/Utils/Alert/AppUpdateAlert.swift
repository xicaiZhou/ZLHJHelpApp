...
extension AppUpdateAlert{
    func initView() {
        ...
        self.backgroundColor = UIColor(red: CGFloat(0 / 255.0), green: CGFloat(0 / 255.0), blue: CGFloat(0 / 255.0), alpha: 0.5 / 1.0)
        //获取更新内容高度
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            ...
            let updatebg_image = UIImageView(frame: CGRect(x: updateView.frame.size.width*0.26/2, y: 50, width: updateView.frame.size.width*0.7, height: updateView.frame.size.height*0.7))
            updatebg_image.image = UIImage(named: "update_bg")
            updateView.addSubview(updatebg_image)
            
            let updateIcon = UIImageView(frame: CGRect(x: updateView.frame.size.width*0.35, y: -Ratio(x: (updateView.frame.size.width*0.7)/8), width: Ratio(x: (updateView.frame.size.width*0.5)/4), height: Ratio(x: (updateView.frame.size.width*0.5)/2)))
            updateIcon.image = UIImage(named: "update_huojian")
            updateView.addSubview(updateIcon)
            
            //版本号
            let versionLabel = UILabel(frame: CGRect(x: updatebg_image.frame.minX+50, y: Ratio(x:120), width: updatebg_image.frame.size.width - 100, height: Ratio(x:28)))
            ...
            let descTextView = UITextView(frame: CGRect(x: updatebg_image.frame.minX+50, y: Ratio(x: 1) + versionLabel.frame.maxY, width: updatebg_image.frame.size.width - 100, height: descHeight))
            descTextView.font = UIFont.systemFont(ofSize: CGFloat(16))
            descTextView.textContainer.lineFragmentPadding = 0
            descTextView.textContainerInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
            ...
            updateButton.backgroundColor = systemColor
            updateButton.frame = CGRect(x: updatebg_image.frame.minX+20, y: descTextView.frame.maxY + 20 + Ratio(x:20), width: updatebg_image.frame.size.width-40 , height: 50)
            ...
            let updateButton = UIButton(type: .system)
            updateButton.backgroundColor = systemColor
            updateButton.frame = CGRect(x: Ratio(x:25), y: descTextView.frame.maxY + 20 + Ratio(x:20), width: updateView.frame.size.width - Ratio(x:50), height: Ratio(x:40))
            updateButton.clipsToBounds = true
            updateButton.layer.cornerRadius = 5.0
            updateButton.addTarget(self, action: #selector(self.updateVersion), for: .touchUpInside)
            updateButton.setTitle("立即更新", for: .normal)
